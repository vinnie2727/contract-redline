...
import { benchmarkResultByIssueId } from "@/lib/benchmark/engine";
...
export default function Home() {
  ...
  const [benchmarkBandFilter, setBenchmarkBandFilter] = useState<
    "all" | "hard" | "borderline" | "acceptable"
  >("all");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`${publicBasePath}/api/benchmark`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            supplierName: ctx.supplier,
            clientName: ctx.buyer,
            ...
            issues: analysis.issues.map((i) => ({
              ...
              problem: i.problem,
              supplierAsk:
                (i.revisedLanguageSummary && String(i.revisedLanguageSummary)) ||
                i.problem ||
                i.clauseTitle ||
                "",
            })),
        ...
        if (!cancelled && data.results) setBenchmarkResults(data.results);
      } catch {
        if (!cancelled) setBenchmarkResults([]);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [analysis, ctx.supplier, ctx.buyer, ctx.contractType]);

  async function saveAnalysisToRepository() {
    if (!analysis || !ctx.contractType) return;
    setSaveMessage(null);
    try {
      const res = await fetch(`${publicBasePath}/api/save-analysis`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contractName:
            ctx.project.trim() ||
            `${ctx.supplier || "Supplier"} – ${ctx.equipment || "Contract"}`,
          supplierName: ctx.supplier,
          clientName: ctx.buyer || "Client",
          contractType: ctx.contractType,
          equipmentType: ctx.equipment,
          signedDate: new Date().toISOString().slice(0, 10),
          dealType: ctx.focus || undefined,
          notes: ctx.priorities || undefined,
          issues: analysis.issues.map((i) => ({
            issueId: i.issueId,
            primaryCategory: i.primaryCategory,
            clauseTitle: i.clauseTitle,
            problem: i.problem,
            action: i.action,
            revisedLanguageSummary: i.revisedLanguageSummary,
          })),
        }),
      });
      ...
      setSaveMessage(
        `Saved to repository: ${data.contract.id} (${data.clausesCreated} clause records).`
      );
    } catch (e: unknown) {
  ...
  async function runAnalysis() {
    ...
    if (!ctx.contractType) {
      setError(
        "Select contract type: Materials / Equipment or Services (required for precedent benchmarking)."
      );
      return;
    }
    setLoading(true);
  ...
  const benchById = useMemo(
    () => benchmarkResultByIssueId(benchmarkResults),
    [benchmarkResults]
  );

  const filteredIssues = (analysis?.issues || [])
    .filter((i) => filter === "All" || i.severity === filter)
    .filter((i) => {
      if (benchmarkBandFilter === "all") return true;
      const br = benchById.get(i.issueId);
      if (!br) return false;
      if (benchmarkBandFilter === "hard") return br.band >= 4;
      if (benchmarkBandFilter === "borderline") return br.band === 3;
      return br.band <= 2;
    });
  ...
  function benchmarkForRiskArea(area: string): BenchmarkResult | undefined {
    const a = (area || "").toLowerCase().trim();
    if (!a) return undefined;
    for (const r of benchmarkResults) {
      const fam = r.clauseFamily.toLowerCase();
      if (fam.includes(a.slice(0, 8)) || a.includes(fam.slice(0, 6))) return r;
    }
    for (const iss of analysis?.issues || []) {
      const blob = `${iss.clauseTitle} ${iss.problem}`.toLowerCase();
      if (blob.includes(a.slice(0, 12))) {
        const b = benchById.get(iss.issueId);
        if (b) return b;
      }
    }
    return undefined;
  ...
  function benchmarkForTradeRow(linkedIds: string[]): BenchmarkResult | undefined {
        ...
        <div className="pt-3">
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-4 py-2.5 text-sm text-left transition-all border-l-[3px] disabled:opacity-35 disabled:cursor-not-allowed disabled:hover:bg-transparent ${tab === item.key ? "text-white border-[#6ea8d9] bg-[rgba(110,168,217,0.1)]" : "text-gray-500 border-transparent hover:text-gray-300 hover:bg-white/5"}`}
            >
                ...
                {ctx.contractType ? (
                  <div className="space-y-3">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-3">
                      ...
                      <button
                        type="button"
                        onClick={() => void saveAnalysisToRepository()}
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="col-span-3">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(
                        ["Materials / Equipment", "Services"] as ContractType[]
                      ).map((t) => (
                        <button
                          key={t}
                          type="button"
                          onClick={() =>
                            setCtx((p) => ({ ...p, contractType: t }))
                          }
                          className={`flex-1 min-w-[140px] py-2 px-3 text-xs font-semibold rounded-md border-2 transition-all ${
                            ctx.contractType === t
              ...
              <div className="flex justify-end items-center gap-3">
                ...
                >
                  {redlineFile || baselineFile
                    ? ctx.contractType
                      ? "✓ Ready to analyze"
                      : "Select contract type to continue"
                  ...
                  <div className="flex gap-1.5 flex-wrap justify-end">
                    {(
                      ...
                      <button
                        key={k}
                        type="button"
                        onClick={() => setBenchmarkBandFilter(k)}
                        className={`px-2.5 py-1 rounded-md text-[11px] font-semibold border transition-all ${
                          benchmarkBandFilter === k
                            ? "bg-[#1a2942] border-[#1a2942] text-white"
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="px-5 pb-2">
                          <BenchmarkPanel
                            benchmark={benchById.get(iss.issueId) ?? null}
                            supplierName={ctx.supplier}
                            contractTypeLabel={
                              ctx.contractType || "Contract type not set"
                            }
                            contractCount={
                              benchById.get(iss.issueId)?.priorDealCount ??
                              benchmarkResults[0]?.priorDealCount ??
                              0
                            }
                          />
                        </div>
                      ...
                      (row, i) => (
                        ...
                        >
                          ...
                          <div className="flex flex-wrap items-center gap-2 shrink-0">
                            {(() => {
                              const bb = benchmarkForRiskArea(row.area);
                              return bb ? <BandPill band={bb.band} /> : null;
                            })()}
                            <span
                      ...
                      <table className="w-full min-w-[640px] text-sm border-collapse">
                        <thead>
                          <tr className="border-b border-slate-100 bg-slate-50 text-left">
                            ...
                            <th className="px-3 py-2.5 text-center text-[10px] font-bold uppercase tracking-wider text-slate-500 w-[100px] align-top">
                              Precedent
                            </th>
                            ...
                            return (
                              ...
                              >
                                ...
                                <td className="px-3 py-3 align-top text-center">
                                  {(() => {
                                    const bb = benchmarkForTradeRow(ids);
                                    return bb ? (
                                      <BandPill band={bb.band} className="mx-auto" />
                                    ) : (
                          ...
                          {nonemptyBatnaRows(
                            ...
                            >
                              <td className="px-3 py-3 align-top font-semibold text-slate-900">
                                <div>{row.topic || "—"}</div>
                                {(() => {
                                  const bb = benchmarkForRiskArea(row.topic || "");
                                  return bb ? (
                                    <div className="mt-1.5 flex justify-start">
                                      <BandPill band={bb.band} />
                                    </div>
                                  ) : null;
                                })()}
                              </td>
